...
import { ArrowLeft, Send, Paperclip, Image as ImageIcon, FileText, Video, X, Check, CheckCheck, Mic, Reply, Trash2, RefreshCw, EllipsisVertical } from "lucide-react";
...
export function ChatWindow({
  ...
  useEffect(() => {
    // Only auto-scroll if it's the first load or user is already at bottom
    // We pass force=true only when the messages array actually changes in a way that implies a NEW message
    // Since polling happens every second, we check if the last message ID is different from what we had.
    scrollToBottom();
  }, [messages]);
  ...
  const renderFilePreview = (message: Message, isSent: boolean) => {
    ...
    if (message.fileType === 'audio/webm' || message.fileType?.startsWith('audio/')) {
      return (
        <div className="mb-2">
    ...
    return (
      <div className={`flex items-center gap-2 mb-2 p-2 rounded ${isSent ? 'bg-[#004a3c]' : 'bg-[#1a252d]'}`}>
  ...
  return (
    <div className="flex flex-col h-full bg-[#0b141a] overflow-hidden relative">
      ...
      <div className="bg-[#202c33] px-4 py-3 flex items-center gap-3 sticky top-0 z-50 shadow-md flex-shrink-0">
        ...
        <Avatar className="w-10 h-10 bg-[#6b7c85]">
          <AvatarFallback className="bg-[#6b7c85] text-white">
            {partnerName.charAt(0).toUpperCase()}
          </AvatarFallback>
        </Avatar>
        <div className="flex-1">
          <h2 className="text-white font-bold">{partnerName}</h2>
          {!isGroup && (
          ...
          {isGroup && isPartnerTyping && (
            <p className="text-[#00a884] text-xs font-bold">Someone is typing...</p>
          )}
        </div>
      ...
      <div className="bg-[#202c33] px-4 py-3">
        ...
        <div className="flex gap-2 items-center">
          ...
          <Button
            ...
            className="bg-[#2a3942] hover:bg-[#1a252d] text-gray-400 hover:text-white"
          ...
          <Input
            ...
            className="flex-1 bg-[#2a3942] border-none text-white placeholder:text-gray-500"
            autoComplete="off"
            autoCorrect="off"
